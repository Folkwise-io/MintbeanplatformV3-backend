...
interface WorkshopRegistrationReminder1EmailVars extends EmailVars {
  user: User;
  meet: Meet;
  users?: never;
  html?: never;
...
export default class WorkshopRegistrationReminder1EmailTemplate implements EmailTemplate {
  ...
  async inflateVars(scheduledEmail: ScheduledEmail): Promise<WorkshopRegistrationReminder1EmailVars> {
  ...
  generateEmails(emailVars: WorkshopRegistrationReminder1EmailVars): Email[] {
    ...
    const email: Email = {
      ...
      subject: `Workshop Tomorrow! ${title}`,
  ...
  dispatch(emailVars: WorkshopRegistrationReminder1EmailVars): Promise<EmailResponse[]> {
