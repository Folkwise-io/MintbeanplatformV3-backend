...
interface HackathonRegistrationReminder2EmailVars extends EmailVars {
  user: User;
  meet: Meet;
  users?: never;
  html?: never;
...
export default class HackathonRegistrationReminder2EmailTemplate implements EmailTemplate {
  ...
  async inflateVars(scheduledEmail: ScheduledEmail): Promise<HackathonRegistrationReminder2EmailVars> {
  ...
  generateEmails(emailVars: HackathonRegistrationReminder2EmailVars): Email[] {
    ...
    const email: Email = {
      ...
      subject: `Hackathonathon starts in 30 mins! ${title}`,
  ...
  dispatch(emailVars: HackathonRegistrationReminder2EmailVars): Promise<EmailResponse[]> {
