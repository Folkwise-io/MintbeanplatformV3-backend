...
interface HackathonRegistrationConfirmEmailVars extends EmailVars {
  user: User;
  meet: Meet;
  users?: never;
  html?: never;
...
export default class HackathonRegistrationConfirmEmailTemplate implements EmailTemplate {
  ...
  generateEmails(emailVars: HackathonRegistrationConfirmEmailVars): Email[] {
    const { user, meet, id } = emailVars;
    const { title } = meet;
    const email: Email = {
      to: user.email,
      from: senderEmail,
      subject: `Hackathonathon Registration Confirmation: ${title}`,
  ...
  dispatch(emailVars: HackathonRegistrationConfirmEmailVars): Promise<EmailResponse[]> {
